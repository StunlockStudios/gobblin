...
/**
 * An abstract implementation of {@link Extractor} for Kafka, where events are
 * in Avro format.
 *
 * Subclasses should implement {@link #getRecordSchema(byte[])} and
 * {@link #getDecoder(byte[])}. Additionally, if schema registry is not used
 * (i.e., property {@link KafkaSchemaRegistry#KAFKA_SCHEMA_REGISTRY_CLASS} is
 * not specified, method {@link #getExtractorSchema()} should be overriden.
 *
 * @author ziliu
 */
@Slf4j
public abstract class NewStunlockKafkaAvroExtractor<K> extends NewStunlockKafkaExtractor<Schema, GenericRecord>
{
	private static final Logger LOG = LoggerFactory.getLogger(NewStunlockKafkaAvroExtractor.class);

	protected static final Schema DEFAULT_SCHEMA = SchemaBuilder.record("DefaultSchema").fields().name("header").type(SchemaBuilder.record("header").fields().name("time").type("long").withDefault(0).endRecord()).noDefault().endRecord();
	...
	public NewStunlockKafkaAvroExtractor(WorkUnitState state)
	{
		super(state);
		LOG.info("NewStunlockKafkaAvroExtractor");
	...
	/**
	 * Get the schema to be used by this extractor. All extracted records that
	 * have different schemas will be converted to this schema.
	 */
	...
	/**
	 * Obtain the Avro {@link Schema} of a Kafka record given the payload of the
	 * record.
	 */
	protected abstract Schema getRecordSchema(byte[] payload);

	/**
	 * Obtain the Avro {@link Decoder} for a Kafka record given the payload of
	 * the record.
	 */
