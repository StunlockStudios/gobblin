 ...
 * @author ziliu
 */
public abstract class NewStunlockKafkaExtractor<S, D> extends EventBasedExtractor<S, D> {

  private static final Logger LOG = LoggerFactory.getLogger(NewStunlockKafkaExtractor.class);
  ...
  protected final NewStunlockKafkaAPI kafkaWrapper;
  ...
  public NewStunlockKafkaExtractor(WorkUnitState state) {
    super(state);
	LOG.info("NewStunlockKafkaExtractor");
    ...
    LOG.info("Instantiation new api!!!!!");
    this.kafkaWrapper = closer.register(new NewStunlockKafkaAPI(state));
